...
lm(X1 ~ X2, data = x) %>% tidy()
lm(X1 ~ X2, data = mcar) %>% tidy()
...
```{r echo=FALSE}
perform %>% group_by(gender) %>% summarise(n = n(),
...
## Regression imputation + residual error | Example
...
```{R echo=FALSE}
mice::md.pattern(weightloss %>% pivot_wider(id_cols = c(id, group), names_from = month, values_from = weight))
