...
- *Intermittend missing data*: missing data on one or more of the measured variables that are used as a predictor, covariate or outcome.
...
The underlying cause of missing data and were first described by Rubin (1976). 
...
In the MCAR example both `X2` and `X3` are not related to the probability of missing data in `X1`, so we may assume that the missing data in `X1` are MCAR. However, in the MAR example, both variables are related tot he probability of missing data in `X1`, so in that case we can assume that the data are not-MCAR. 
...
## Assuming the missing data mechanism {.smaller}
