...
## FIML in `lavaan` descriptives {.smaller}
...
## FIML in `lavaan` correlation {.smaller}
...
```{r echo=FALSE}
summary(fit, header = FALSE, standardized = TRUE)
```
...
# Missing data in questionnaires {data-background=Puzzle_zgnb3.jpg data-background-size=cover}
...
<br>
<center><img src="images/2978479.jpg" height="320px" /></center>
...
## Illustration item and total scores {.smaller .build}
...
## Example no imputation {.smaller .build}
<div>
...
* Missing in dependent variable are handled in multilevel analysis model
* Regular multiple imputation to wide data when possible
* Multilevel multiple imputation to deal with missing covariates when design is more complicated for wide imputation