...
First select the variables from the `fdd` data for this assignment.
...
To analyze the data with a longitudinal multilevel model (preferred, however repeated measurements model can be used too), then we need to restructure the data into a long format. The repeated measurement are the follow-up time, and the outcome variable is a post-traumatic stress (PTSD) score. 
...
Apply the longitudinal model.

```{r}
model <- lme4::lmer(ptsd ~ time + age + sex + cbcl1+ (1|id), data = fdd1_long)
summary(model)
...
**c. Perform a multilevel multiple imputation with only a random intercept, use 50 imputations and 10 iterations. Repeat the longitudinal analysis, compare the parameter estimates, with the estimates from the previous answer.**

The multilevel imputation should be performed on the long data, so we use the restructured dataset to make a predictormatrix.
...
Initialize the imputation model to change the method to "2l.pmm" to use a multilevel imputation model for the variables with missing data (trauma and ptsd). 
...
Run the imputation, with 50 imputations and 10 iterations using the predictormatrix with -2 for "id",  and the "2l.pmm" methods for the variables that have missing values. 

```{r}

imp_long <- mice(fdd1_long, pred = pred, meth = long_meth, m = 50, maxit = 10)

```

Fit the model to each imputated dataset and pool the results. 

```{r}

fit <- with(imp_long, lme4::lmer(ptsd ~  time + age + sex + (1|id) ))
            
summary(pool(fit))
```


The results show a similar conclusion; the ptsd scores decrease over time for all participants, however the coefficients seem less strong after imputation. One possible reason might be that the itention to treat analysis (analysis with imputations) reveals that mostly participants with less progress (less decrease in PTSD score) drop-out. Further investigations into the missing data and other information in the data may give us more insight into this. 
