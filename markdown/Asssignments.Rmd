...
> a. Repeat the example from the lecture on FIML and estimate the descriptive means for the `airquality` variables Ozone, Solar.R, Wind and Temp using FIML esimation. 
...
> *Use the builtin `fdd` data for this assignment and select only the following variables: "id", "trt", "age", "sex", "trauma", "yc1", "yc2", "yc3". See `?fdd` for more information about the dataset.

> a. Which variables have missing data and how many?

> b. Analyze the data with a longitidunal model (yc1, yc2 and yc3 are the dependent repeated measurements), using trt, age, sex and trauma as predictor.

> c. Apply multiple imputation to the wide data an repeat the longitudinal analysis, compare the result with the previous answer. 

> d. Perform a multilevel multiple imputation to deal with the missing values and repeat the longitudinal analysis again, compare the results with the previous answers. 
